...
//IO方向设置
#define CT_SDA_IN()  {GPIOC->MODER&=~(3<<(2*5));GPIOC->MODER|=0<<2*5;}	//PF11输入模式
#define CT_SDA_OUT() {GPIOC->MODER&=~(3<<(2*5));GPIOC->MODER|=1<<2*5;} 	//PF11输出模式
//IO操作函数	 
#define CT_IIC_SCL    PBout(0) 	//SCL
#define CT_IIC_SDA    PCout(5) //SDA	 
#define CT_READ_SDA   PCin(5)  //输入SDA 
